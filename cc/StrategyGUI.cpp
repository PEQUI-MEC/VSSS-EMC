...
void StrategyGUI::_event_saveFormation_bt_clicked() {
	...
	for (int i = 0; formationName[i] != '\0'; i++) {
		if (!(formationName[i] >= 'a' && formationName[i] <= 'z') &&
			!(formationName[i] >= 'A' && formationName[i] <= 'Z')) {
			formationName[i] = '_';
		}
	}
	...
	if (txtFile.is_open() && tmpTxtFile.is_open()) {
		...
		for (int i = 0; i < 3; i++) {
			...
			tmpTxtFile << formation_positions[i].x << " " << formation_positions[i].y << " "
					   << formation_orientations[i] << " ";
		}
		tmpTxtFile << std::endl;
