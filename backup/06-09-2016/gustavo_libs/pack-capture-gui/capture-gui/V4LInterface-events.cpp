...
//			}
	...
	reset_warp_flag=true;
		...
		void V4LInterface::__event_bt_HSV_calib_clicked(){
	std::cout<<"HSV calib"<<std::endl;
	
	if (HScale_Hmin.get_state() == Gtk::STATE_ACTIVE)
	{
	...
	else
	{
		...
		bt_HSV_calib.set_state(Gtk::STATE_ACTIVE);
	}
